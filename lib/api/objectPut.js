...
function objectPut(authInfo, request, streamingV4Params, log, callback) {
    ...
    (err, bucket, objMD) => {
        ...
        return async.waterfall([
            ...
            console.log("About to collect default metrics");
            debugger;
            client.collectDefaultMetrics();
            console.log("collected metrics. initiating counter;");
            var requestCounter = new client.Counter({
                name: 'http_requests_total',
                help: 'Total number of HTTP requests made.',
                labelNames: ['code', 'handler', 'method'],
            });
            console.log("counter initiated. sending beacon with ",
                request, request.url, request.method
            );
            requestCounter.inc({
                'code': "100", //"res.statusCode",
                'handler': url.parse(request.url).pathname,
